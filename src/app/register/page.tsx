...
function RegistrationPageContent() {
  ...
  const initialTier =
    (searchParams.get("tier") as keyof typeof tierDetails) || "pro";

  const [selectedTier, setSelectedTier] =
    useState<keyof typeof tierDetails>(initialTier);
  ...
  // In the onSubmit function of RegistrationPageContent component:
  const onSubmit = async (data: FormData) => {
    setIsSubmitting(true);

    try {
      // 1. First, register the user in backend
      const registrationResponse = await fetch(
        "http://localhost:5000/api/auth/register",
        {
          ...
          body: JSON.stringify({
            ...
            phone: data.phone,
            address: "To be updated", // You might want to add address field
            tier:
              data.tier === "starter"
                ? "free"
                : data.tier === "pro"
      ...
      // 2. Handle tier-based redirection

      setTimeout(() => {
        if (data.tier === "starter") {
          router.push("/dashboard");
        } else {
          // Store payment data including the authorization URL
          const paymentData = {
            userId: registrationResult.user._id,
            email: data.email,
            tier: data.tier,
            amount: tierDetails[data.tier].price,
            reference: registrationResult.paymentData?.reference,
            authorization_url:
              registrationResult.paymentData?.authorization_url,
            access_code: registrationResult.paymentData?.access_code,
          };
          ...
          router.push(
            `/payment?tier=${data.tier}&amount=${
              tierDetails[data.tier].price
            }&userId=${registrationResult.user._id}`
          );
        }
      }, 1500);
        ...
        <div className="w-full lg:w-7/12 xl:w-1/2 h-full flex flex-col p-6 lg:p-12 overflow-y-auto scrollbar-hide">
          ...
          <div className="mb-8">
            <div className="flex justify-between text-sm font-medium mb-2">
              <span className={step === 1 ? "text-primary" : "text-white/40"}>
                Step 1: Account Info
              </span>
              <span className={step === 2 ? "text-primary" : "text-white/40"}>
          ...
          <div className="flex-1 flex flex-col justify-center max-w-lg mx-auto w-full">
            ...
            <div className={`step-content ${step === 1 ? "active" : ""}`}>
              ...
              <p className="text-white/60 mb-8">
                Join Nigeria's fastest growing marketplace. Start selling in
                minutes.
              </p>
              <form
                onSubmit={(e) => {
            ...
            <div className={`step-content ${step === 2 ? "active" : ""}`}>
              ...
              <p className="text-white/60 mb-8">
                Choose the package that fits your business scale. You can change
                this later.
              </p>
          ...
          <div className="relative max-w-md w-full space-y-12">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="glass p-6 rounded-2xl border border-white/5 transform hover:-translate-y-1 transition-transform duration-300 delay-150">
                <span className="material-symbols-outlined text-3xl text-green-500 mb-3">
                  verified_user
                </span>
                <div className="text-2xl font-bold font-display text-white">
                  100%
              ...
              <div className="glass p-6 rounded-2xl border border-white/5 transform hover:-translate-y-1 transition-transform duration-300 delay-200">
                <span className="material-symbols-outlined text-3xl text-yellow-500 mb-3">
                  star
                </span>
                <div className="text-2xl font-bold font-display text-white">
                  4.8/5
            ...
            <div className="bg-gradient-to-r from-white/5 to-transparent p-6 rounded-2xl border-l-4 border-primary">
              ...
              <p className="text-lg italic text-white/90 mb-4 font-display">
                "WeSellAll transformed my small shoe business in Lagos. Within 3
                months, my sales tripled. The platform is incredibly easy to
                use."
              </p>
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-start gap-3">
                ...
                <p className="text-sm text-white/70">
                  Instant access to millions of buyers across Nigeria.
                </p>
              </div>
              <div className="flex items-start gap-3">
                <span className="material-symbols-outlined text-primary mt-0.5">
                ...
                <p className="text-sm text-white/70">
                  24/7 dedicated support team to help you grow.
                </p>
              </div>
              <div className="flex items-start gap-3">
                <span className="material-symbols-outlined text-primary mt-0.5">
